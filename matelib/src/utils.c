...
void paquete(int conexion)
{
	...
	FILE *f = fopen("tp0.log","r");
	while (fgets(leido,sizeof(leido),f) != NULL) {
		agregar_a_paquete(paquete,&leido,100);
	}
...
t_config* leer_config(char *unaConfig)
{
	t_config *config;
	if((config = config_create(unaConfig)) == NULL) {
		printf("No pude leer la config\n");
		exit(2);
	}
	return config;
